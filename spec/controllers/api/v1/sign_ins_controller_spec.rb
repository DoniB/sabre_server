...
RSpec.describe Api::V1::SignInsController, type: :controller do
  ...
  it "should not return the user info or token with inactive user" do
    user = create(:user, password: "abc123", active: false)
    post :create, params: { email:    user.email,
                            password: "abc123" }
    json = JSON.parse(response.body)
    expect(json["error"]).to_not be_nil
  ...
  it "should not return the user info or token with inactive admin" do
    user = create(:admin, password: "abc123", active: false)
    post :create, params: { email:    user.email,
                            password: "abc123" }
    json = JSON.parse(response.body)
    expect(json["error"]).to_not be_nil
